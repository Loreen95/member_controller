    ...
    def member_add_cmd(self, request, _, char):
        if char.char_id:
            if self.get_member(char.char_id):
                self.bot.send_private_message(request.sender.char_id, "<highlight>%s<end> is already a member." % char.name)
            else:
                self.add_member(char.char_id)
                char_name = self.pork_service.get_character_info(char.char_id)
                self.points_controller.bank_create_cmd(request, None, senderObj(char.char_id, char_name))
                char_info = self.pork_service.get_character_info(char.char_id)
                self.bot.send_private_channel_message("<highlight>%s<end> <white>(%d/<end><green>%d)<end> <highlight%s<end> <white>from<end> <highlight>%s<end> has been invited by <highlight>%s<end>" % (char.name, char_info.level, char_info.ai_level, char_info.profession, char_info.org_name, request.sender.name))
                self.private_channel_service.invite(char.char_id)
                self.bot.send_private_message(request.sender.char_id, "You have added <highlight>%s<end> as member, and invited to the private channel." % (char.name))
        else:
            self.bot.send_private_message(request.sender.char_id, "Could not find character <highlight>%s<end>.") % char.name
    ...
    def member_remove_cmd(self, request, _, char):
        if char.char_id:
            if self.get_member(char.char_id):
                ...
                self.bot.send_private_message(request.sender.char_id, "<highlight>%s<end> has been removed as a member." % char.name)
            else:
                self.bot.send_private_message(request.sender.char_id, "<highlight>%s<end> is not a member." % char.name)
        else:
            self.bot.send_private_message(request.sender.char_id, "Could not find character <highlight>%s<end>." % char.name)
